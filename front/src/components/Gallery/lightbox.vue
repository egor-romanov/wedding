<template>
    <div class="lb" v-if="items.length>0">
        <div class="lb-grid" :class="[css,items.length>cells?'lb-grid-' + cells: 'lb-grid-' + items.length]">
            <div class="lb-item" v-for="(src, i) in items" @click="show(i)" v-bind="i" v-if="i<cells" :style="bg(src)">
                <span class="lb-more" v-if="i==cells-1 && items.length - cells>0">{{ items.length - cells}}+</span>
            </div>
        </div>

        <transition enter-active-class="animated fadeIn" leave-active-class="animated fadeOut">
            <div class="lb-modal" v-if="index>=0" @click="close">
                <button class="btn btn-outline-danger btn-sm lb-modal-close" @click.self="close">
                    <span> X </span>
                </button>
                <button class="btn-outline-primary lb-modal-prev" @click.self="prev">
                    &lt;
                </button>
                <button class="btn-outline-primary lb-modal-next" @click.self="next">
                    >
                </button>

                <div class="lb-modal-img">
                    <img :src="src" @click.self="next">
